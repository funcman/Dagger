...
bool DIniFile::LoadMemory(DMemory* memory) {
    Free();
    InitList((char*)memory->GetMemPtr(), (long)memory->GetMemLen());
    return true;
}
...
DWORD DIniFile::GetSaveSize() {
    DWORD size = 0;
    DIniSecNode* sec = (DIniSecNode*)GetHead();
    while (sec) {
        size += strlen(sec->name_) + 10;
        DIniKeyNode* key = (DIniKeyNode*)sec->keyList_.GetHead();
        while (key) {
            size += strlen(key->name_) + strlen(key->value_) + 10;
            key = (DIniKeyNode*)key->GetNext();
        }
        size += 4;
        sec = (DIniSecNode*)sec->GetNext();
...
void DIniFile::InitList(char* buffer, long len) {
    char line[1024] = "";
    char section[256] = "";
    int idx = 0;
    int lineLen = 0;
...
bool DIniFile::IsKeyChar(BYTE c) {
    return (c >= 33 && c <= 126) || (c >= 0xa1 && c <= 0xfe);
}

char* DIniFile::SplitKeyValue(char* str) {
    while (*str) {
        if (*str == '=') {
            *str = 0;
            return str + 1;
...
DIniKeyNode* DIniFile::SetKeyValue(char const* section, char const* key, char const* value) {
    ...
    DFree(k->value_);
    k->value_ = (char*)DCAlloc(strlen(value) + 2);
    strcpy(k->value_, value);
    return k;
}
