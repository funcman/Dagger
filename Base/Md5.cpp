...
void DMd5(unsigned char* output, unsigned char* input, int len) {
    MD5_CTX context;
    ...
    for (int i = 0; i < 16; i++) {
        *output++ = Hex[digest[i] >> 4];
        *output++ = Hex[digest[i] & 0x0f];
    }
    *output = 0;
}

void DMd5String(char* output, char* input) {
    MD5_CTX context;
    memset(&context, 0, sizeof(MD5_CTX));
    MD5Init(&context);
...
static void MD5Update(MD5_CTX* context, unsigned char* input, int inputLen) {
    ...
    if ((context->iCount[0] += ((UINT4)inputLen << 3)) < ((UINT4)inputLen << 3))
        context->iCount[1]++;
